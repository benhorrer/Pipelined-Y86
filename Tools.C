...
{
   int outOfBoundsFlag = (byteNum < 8 && byteNum > -1);
   source = source >> (byteNum * 8);
   uint64_t selectByte = (source & 0xff);
   return selectByte * outOfBoundsFlag;
}
...
{
   int outOfBoundsFlag = (byteNum <= 7 && byteNum >= 0);
   uint64_t mask = 0xff;
   mask = mask << (8 * byteNum);
   mask = mask * outOfBoundsFlag;
   return source | mask;
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
   ...
   op1 = sign(op1);
   op2 = sign(op2);
   bool sameSign = (op1 == op2);
   bool isDifferent = (sign(sum) != op1);
   return (sameSign && isDifferent);
}
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
   ...
   op1 = sign(op1);
   op2 = sign(op2);
   bool differentSign = (op1 != op2);
   bool isSame = (sign(diff) == op1);
   return (differentSign && isSame);
}
